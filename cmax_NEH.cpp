...
int main() {
	...
	for (int i = 0; i < n_tasks; i++)//liczenie sumy czasow wykonania na wszyskich maszynach dla danego zadania
	{
	    ...
	    for(int n=0; n<n_tasks;n++){if(n!=i){if(wartosc == temp_tablica_czasow[n]){wartosc = wartosc + 98765;};};}
    ...
    for (int i = 0; i < n_tasks; i++) 
    {if(temp_tablica_czasow[i] > 98000){cout << tablica_indeksow[i][0] << " " << temp_tablica_czasow[i] - 98765<< "\n";}
    else{
   ...
   //wyswietlenie sum czasow dla poszczegolnych zadan
	 ...
	 for (int i = 0; i < n_tasks; i++) 
    {
        ...
        {
            if(tablica_indeksow[x][1] == temp_tablica_czasow[i] && tablica_indeksow[x][1] < 98765)
            {
                numer = tablica_indeksow[x][0];
                tablica_kolejnosci[i] = numer;
              //  cout << "aaaa " << tablica_indeksow[x][1] << "  " << x<< "\n";
            }
            else if(tablica_indeksow[x][1] > 98000 && tablica_indeksow[x][1]-98765 == temp_tablica_czasow[i])
            {
	...
	{ 
		...
		cout << "\n";	cout << "\n";
	int maxy[n_tasks];
	for(int h=1;h<=n_tasks;h++){int maxcmax=999999;
	do {
