...
int findB (int U, vector<Zadanie> wek, vector<int> k)
{cout << "cmax(kolejnosc,wek,1) : " << endl;

    vector<int> kolejnosc = k;
    for(int i=wek.size()-1;i>=0;i--)
    {
    cout << cmax(kolejnosc,wek,1) << "   " << wek[kolejnosc[i]].q << " dada " << U << endl;
        if(((cmax(kolejnosc,wek,1))+wek[kolejnosc[i]].q)==U) //ten cmax pierwszy jest chyba zle
        {
            return i;
        }
    }
    cout << "nie przeszlo B" << endl;
    return 0;
}
 
int findA(int U,int b, vector<int> kolejnosc,vector <Zadanie> wek)
{
    int tmp;
    for(int i=0;i<b;i++)
    {
        ...
        for(int d=i;d<=b;d++)
        {
            tmp+=wek[kolejnosc[d]].p;
        }
        if(wek[kolejnosc[i]].r+tmp+wek[kolejnosc[b]].q==U)
        {
            return i;
        }
    }
    cout << "nie przeszlo A" << endl;
    return 0;
}
...
int Carlier(vector<Zadanie> wek,int l_Zadan){
    ...
    U = cmax(kolejnosc, wek, l_Zadan);//1 correct
    if(U<UB){ // 2 correct
        UB=U;//3 correct
        kolejnosc2= kolejnosc;//4 correct
    }
    b=findB(U,wek,kolejnosc);//6 correct
    cout<<"b : " << b << endl;
    a=findA(U,b,kolejnosc,wek);// 7 correct
    cout<<"a : " << a << endl;
    for(i=b; i>=a;i--) // 8 correct
    {
 ...
 cout<<"c : " << c << endl;
    if(c=0) //9 correct
        return cmax(kolejnosc2,wek,l_Zadan); //10 correct
 
    for(int x=0;x<(c+1-b);x++) //12 correct
        K[x]=c+1;
 
 
        for(int y=c+1; y<b;y++){ //13 correct
            j=znajdz_min(wek,K,'r');
            j.p =j.p+wek[kolejnosc[y]].p;
            j=znajdz_min(wek,K,'q');
        }
 
        tempR=wek[kolejnosc[c]].r;
        wek[kolejnosc[c]].r=max(wek[kolejnosc[c]].r, j.r+j.p);// 14 correct
        LB=schragePMTN(wek);//15 correct
        LB=maximum((j.r+j.p+j.q),(j.r+j.p+j.q+c),LB);// 16 correct
        if(LB<UB) //17 correct
            Carlier(wek,l_Zadan);//18 correct
        wek[kolejnosc[c]].r=tempR;//20 correct
 
        tempQ=wek[kolejnosc[c]].q;
        wek[kolejnosc[c]].q=max(wek[kolejnosc[c]].q,(j.p+j.q));//21 correct
        LB=schragePMTN(wek);//22 correct
        LB=maximum((j.r+j.p+j.q),(j.r+j.p+j.q+c),LB);//23 correct
        if(LB<UB)//24 correct
            Carlier(wek,l_Zadan);//25 correct
        wek[kolejnosc[c]].q=tempQ;//27 correct;
        return cmax(kolejnosc2,wek,l_Zadan);
